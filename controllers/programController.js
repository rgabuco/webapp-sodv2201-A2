const Program = require('../models/ProgramCourses');
// get all programs
...
exports.updateCourse =async (req,res)=>
    {
        ...
        try
        {
            ...
            await program.save();
            res.status(200).json({ message: "Course updated successfully", program });
        } 
        catch (error) 
        {
...
exports.deleteCourse = async (req, res) => 
    {
        ...
        try 
        {
            ...
            const courseIndex = program.courses.findIndex(course => course.code === courseCode);
        if (courseIndex === -1) {
            return res.status(404).json({ message: "Course not found" });
        }
        ...
        await program.save();
        res.status(200).json({ message: "Course deleted successfully", program });
        } 
        catch (error) 
        {
